...
if __name__ == '__main__':
    ...
    while CounterT < 5:
        AllMiSequence = []
        ReadMyCsv1(AllMiSequence, 'SM2miRAllMiSequence.csv')
        miRNAEmbedding = []
        ReadMyCsv3(miRNAEmbedding, 'miRNAEmbedding.csv')

        AllDrugCanonicalSMILES = []
        ReadMyCsv1(AllDrugCanonicalSMILES, 'SM2miRAllDrugSmiles.csv')
        DrugEmbedding = []
        ...
        ReadMyCsv1(PositiveSample_Train, 'data/PositiveSample_Train'+str(CounterT)+'.csv')
        PositiveSample_Validation = []
        ReadMyCsv1(PositiveSample_Validation, 'data/PositiveSample_Validation'+str(CounterT)+'.csv')
        PositiveSample_Test = []
        ReadMyCsv1(PositiveSample_Test, 'data/PositiveSample_Test'+str(CounterT)+'.csv')

        NegativeSample_Train = []
        ReadMyCsv1(NegativeSample_Train, 'data/NegativeSample_Train'+str(CounterT)+'.csv')
        NegativeSample_Validation = []
        ReadMyCsv1(NegativeSample_Validation, 'data/NegativeSample_Validation'+str(CounterT)+'.csv')
        NegativeSample_Test = []
        ReadMyCsv1(NegativeSample_Test, 'data/NegativeSample_Test'+str(CounterT)+'.csv')
        ...
        ModelName = 'result/my_model' + str(CounterT) + '.h5'
        ...
        while counter < len(ModelTestOutput):
            ...
            counter = counter + 1
        LabelPredictionProbName = 'result/RealAndPredictionProbA+B' + str(CounterT) + '.csv'
        StorFile(LabelPredictionProb, LabelPredictionProbName)
        LabelPredictionName = 'result/RealAndPredictionA+B' + str(CounterT) + '.csv'
